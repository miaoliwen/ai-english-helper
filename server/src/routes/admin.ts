...
import { detectApiFormat, fetchModelList, ApiFormat } from '../utils/model-api.js';
...
export default async function adminRoutes(app: FastifyInstance) {
  ...
  app.post('/api/admin/providers', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.put('/api/admin/providers/:id', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.delete('/api/admin/providers/:id', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    try { await deleteProvider(id); providerCache.clear(); return { success: true }; }
    catch { return reply.status(404).send({ error: 'Provider not found' }); }
  });

  app.post('/api/admin/providers/:id/test', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const provider = await getProviderWithKey(id);
    if (!provider) return reply.status(404).send({ error: 'Provider not found' });
    try {
      const format = provider.format === 'auto' ? await detectApiFormat(provider.baseUrl) : provider.format as ApiFormat;
      const result = await fetchModelList(provider.baseUrl, provider.apiKey, format);
      return { success: true, message: '连接成功', models: result.models.map((m: { id: string }) => m.id) };
    } catch (e: any) { return { success: false, message: e.message || '连接失败' }; }
  });

  app.get('/api/admin/providers/:id/models', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const provider = await getProviderWithKey(id);
    if (!provider) return reply.status(404).send({ error: 'Provider not found' });
    try {
  ...
  app.post('/api/admin/models', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.put('/api/admin/models/:id', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
  ...
  app.delete('/api/admin/models/:id', { preHandler: [requireAdmin] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
