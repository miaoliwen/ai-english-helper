  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
      ...
      <div class="relative">
        <input v-model="searchQuery" @input="handleSearch" type="text" placeholder="搜索收藏..."
               class="input-field pl-11 pr-4 w-72" />
      ...
      <div v-for="item in filteredItems" :key="item.id"
        ...
        <div class="flex items-start justify-between mb-4">
          ...
          <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button @click.stop="exportItem(item)" class="p-1.5 text-neutral-300 hover:text-accent-600 hover:bg-accent-50 rounded-lg transition-all" title="导出 Markdown">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/>
              </svg>
            </button>
            <button @click.stop="deleteItem(item.id)" class="p-1.5 text-neutral-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="删除">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    <Transition name="scale">
      <div v-if="selectedItem" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="selectedItem = null">
        ...
        <div class="relative bg-white rounded-4xl shadow-2xl w-full max-w-3xl max-h-[80vh] overflow-hidden">
          <div class="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <h3 class="font-semibold text-neutral-900 dark:text-neutral-100">{{ selectedItem.title }}</h3>
            </div>
            <div class="flex items-center gap-2">
              <ExportPanel :content="selectedItem.content" :title="selectedItem.title" />
              <button @click="selectedItem = null" class="p-2 text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl transition-all">
