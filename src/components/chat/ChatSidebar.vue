<template>
  <div class="card-surface p-4 h-full flex flex-col">
    <button @click="emit('new-chat')" class="btn-primary w-full flex items-center justify-center gap-2 mb-4">
    ...
    <div v-if="currentOCR" class="p-3 bg-accent-50 rounded-2xl mb-4">
      <p class="text-xs font-semibold text-accent-700 mb-1">已关联识别结果</p>
      <p class="text-sm text-neutral-700 dark:text-neutral-300 line-clamp-2">{{ currentOCR.text.slice(0, 50) }}...</p>
    </div>
