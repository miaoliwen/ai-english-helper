<template>
  <div v-if="sessions.length === 0" class="text-center text-sm text-neutral-400"
       :class="variant === 'mobile' ? 'py-10 px-4' : 'py-6 px-2'">
    还没有历史对话
  </div>
  <div v-else :class="variant === 'mobile' ? 'space-y-0.5' : 'space-y-1'">
    <div v-for="session in sessions" :key="session.id"
      ...
      <button @click="emit('select', session.id)" class="flex-1 text-left min-w-0">
        <p class="truncate font-medium">{{ session.title || '未命名对话' }}</p>
        <p class="text-xs text-neutral-400 truncate">{{ lastMessagePreview(session) }}</p>
      </button>
      <button @click.stop="emit('remove', session.id)"
              :class="[
              ...
              aria-label="删除对话">
        <svg :class="variant === 'mobile' ? 'w-4 h-4' : 'w-3.5 h-3.5'" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
